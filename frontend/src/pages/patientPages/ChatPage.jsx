...
const ChatPage = () => {
  ...
  useEffect(() => {
    if (selectedChat) {
      const fetchMessages = async () => {
        try {
          const response = await fetch(`${process.env.REACT_APP_API}/api/chats/${selectedChat}/messages`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          const data = await response.json();
  ...
  const startChat = async (user) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API}/api/chats/start`, {
        method: "POST",
        headers: {
  ...
  const sendMessage = async () => {
    ...
    try {
      const sendMessageResponse = await fetch(`${process.env.REACT_APP_API}/api/chats/${selectedChat}/message`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        ...
        <div className="space-y-2 h-full">
          {filteredUserList.length > 0 ? (
            filteredUserList.map((user) => (
              ...
              >
                <img
                  src={`${process.env.REACT_APP_API}/${user.profileImage || userImage}`}
                  alt="avatar"
